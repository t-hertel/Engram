...
def main():
    ...
    parser.add_argument("--layers", type=str, default="1,8,16,23")
    ...
    optimizer = torch.optim.AdamW(model.get_engram_params(), lr=args.lr, weight_decay=0.01, betas=(0.9, 0.95))
    ...
    print(f"{'Step':>6} {'Loss':>9} {'Avg100':>9} {'LR':>10} {'tok/s':>8} {'ms/step':>8} {'GPU(GB)':>8}")
    print("-" * 72)
    ...
    for step in range(start_step, args.steps):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.get_engram_params(), 1.0)
        optimizer.step()
        optimizer.zero_grad()
        loss_val = loss.item()
        ...
        if step % args.log_every == 0 or step == start_step:
            line = f"{step:>6} {loss_val:>9.4f} {avg100:>9.4f} {lr:>10.6f} {toks_per_sec:>8.0f} {step_ms:>8.1f} {gpu_mem:>8.2f}"
            print(line)
        ...
        log_entry = {
            "step": step, "loss": loss_val, "avg100": avg100, "lr": lr,
        ...
        if step % args.save_every == 0 and step > start_step:
            ckpt_data = {
                "engram_state": {k: v.cpu() for k, v in model.engram_modules.state_dict().items()},
                "optimizer": optimizer.state_dict(),
                "step": step, "loss": loss_val, "losses_history": losses_history,
                "config": {
                    "layers": layer_ids, "lr": args.lr, "seq_len": args.seq_len,
                    "batch_size": args.batch_size, "name": args.name, "model": model_name,
                    "domains": domains, "multiplier": args.multiplier,
                },
            }
    ...
    torch.save({
        "engram_state": {k: v.cpu() for k, v in model.engram_modules.state_dict().items()},
        "optimizer": optimizer.state_dict(),
        "step": args.steps, "loss": losses_history[-1], "losses_history": losses_history,
        "config": {
            "layers": layer_ids, "lr": args.lr, "seq_len": args.seq_len,
            "batch_size": args.batch_size, "name": args.name, "model": model_name,
            "domains": domains, "multiplier": args.multiplier,
    ...
    config_out = {
        ...
        "lr": args.lr, "warmup": args.warmup, "trainable_params": trainable,
